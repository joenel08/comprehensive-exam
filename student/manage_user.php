...
if (isset($_GET['id'])) {
	...
	$user = $conn->query("SELECT * FROM student_list sl
	left join program p on p.program_id = sl.program
			...
			<div class="col-sm">
				<div class="form-group">
					...
					<select name="level" id="level" class="form-control" required>
						<option value="masteral" <?= (isset($meta['level']) && $meta['level'] == 'masteral') ? 'selected' : '' ?>>
							Masteral</option>
						<option value="doctorate" <?= (isset($meta['level']) && $meta['level'] == 'doctorate') ? 'selected' : '' ?>>Doctoral</option>
					</select>
				...
				<div class="form-group">
					...
					<select name="program" id="program" class="form-control" required>
						...
						while ($row = $programs->fetch_assoc()):
							$selected = (isset($meta['program']) && $meta['program'] == $row['program_id']) ? 'selected' : '';
							?>
							<option value="<?= $row['program_id'] ?>" <?= $selected ?>>
								<?= $row['program_abbrv'] . ' - ' . $row['programFullDesc'] ?>
							</option>
						<?php endwhile; ?>
					</select>
				</div>
